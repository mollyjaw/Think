...
local Games = {
    ...
    ["phantomforces"] = {
        placeIds = {292439477},
        keywords = {"phantom"},
        url = "https://raw.githubusercontent.com/your-repo/scripts/main/phantomforces.lua"
    },
    ["badbusiness"] = {
        placeIds = {1168263273},
...
function GameDetector.load()
    ...
    if not scriptUrl then
        rconsoleprint("[❌] Game not recognized. PlaceId: " .. game.PlaceId .. "\n")
        return false
    end
    ...
    if success then
        rconsoleprint("[✅] Loaded script from: " .. scriptUrl .. "\n")
    else
        rconsoleprint("[❌] Failed to load script from " .. scriptUrl .. ": " .. tostring(err) .. "\n")
    end
